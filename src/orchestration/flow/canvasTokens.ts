...
/**
 * Empty demo draws its own Demo nav inside the node, so it needs no title
 * clearance above the box.
 */
export const DEMO_TITLE_ALLOWANCE = 0
/** Footer copy sits inside the demo node, below the white card. */
export const DEMO_FOOTER_GAP = 0
export const DEMO_FOOTER_H = 0
export const DEMO_FOOTER_ALLOWANCE = 0
...
/**
 * Empty-file educational demo: Figma composition (nav + 1488×602 card + footer).
 * World size stays fixed so the tablet cannot jump when the walk changes step.
 * Width includes arrow overhang (81px assets, half outside the card).
 */
export const DEMO_ENCLOSURE_W = 1570
export const DEMO_ENCLOSURE_H = 770
...
/**
 * Empty-demo zoom floor. Nav and body copy are authored at 14–24px; below this
 * the walk stops being readable. Map and Outline are disabled while the demo
 * is up. A started journey uses MIN_ZOOM as before.
 */
export const DEMO_MIN_ZOOM = 0.65
